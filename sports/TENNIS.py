...
Route shortcuts, ESPN scoreboard fetch for individual matchups,
and full /tennis-results rendering pipeline.
"""
...
def render_sport_results_page(sport: str, *, season_start_dt=None):
    """Render /tennis-results — individual-sport pipeline."""
    m = main()
    if sport != SPORT:
        return None
    ...
    if min_live and datetime.now() < min_live:
        ...
        return m._results_fallback_page(
            sport,
            f"{m.SPORTS[sport]['name']} results will appear once matches begin on {launch_txt}.",
        )
...
def _normalize_tennis_card_markets(pred: dict) -> None:
    _normalize_moneyline_only_card_markets(pred, sport='TENNIS')
