...
Route shortcuts, ESPN scoreboard fetch for tournament pairings,
and full /golf-results rendering pipeline.
"""
...
def render_sport_results_page(sport: str, *, season_start_dt=None):
    """Render /golf-results — individual-sport pipeline."""
    m = main()
    if sport != SPORT:
        return None
    ...
    if min_live and datetime.now() < min_live:
        ...
        return m._results_fallback_page(
            sport,
            f"{m.SPORTS[sport]['name']} results will appear once events begin on {launch_txt}.",
        )
