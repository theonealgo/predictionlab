...
Route shortcuts and full results-page rendering for NCAA Division I
women's basketball, extracted from the generic _render_daily_sport_results_page
pipeline in NHL77FINAL.py.
...
def register_routes(app) -> None:
    """Register NCAAW-only Flask shortcuts."""
    register_shortcut(app, '/ncaaw', PICKS_SLUG)


def update_ncaaw_scores() -> None:
    """Fetch and update NCAAW scores via ESPN."""
    main().update_espn_scores(SPORT)


def render_sport_results_page(sport: str, *, season_start_dt=None):
    """Render /ncaaw-results — full generic results pipeline."""
    if sport != SPORT:
        return None
