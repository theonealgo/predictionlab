...
Route shortcuts, ESPN scoreboard fetch for UFC bouts,
and full /ufc-results rendering pipeline.
"""
...
from sports._individual_sport import (
    build_graded_daily_results,
    fetch_espn_api_games,
    load_upcoming_games_with_dates,
)
...
def _apply_ufc_results_overlay(html: str) -> str:
    ...
    try:
        ...
        return apply_ufc_isolation_html(html, which="results")
    except Exception as e:
        try:
            m = main()
            m.logger.exception("UFC results isolation overlay failed: %s", e)
        except Exception:
            pass
        return html
...
def render_sport_results_page(sport: str, *, season_start_dt=None):
    """Render /ufc-results — individual-sport pipeline."""
    m = main()
    if sport != SPORT:
        return None
    ...
    if min_live and datetime.now() < min_live:
        ...
        return m._results_fallback_page(
            sport,
            f"{m.SPORTS[sport]['name']} results will appear once fights begin on {launch_txt}.",
        )
    ...
    # ── Individual sport grading (walk-forward Elo from ESPN) ───────────
    _indiv_daily = build_graded_daily_results(sport, season_start_dt, season_end_dt)

    if not completed_games and not _indiv_daily:
        stub = m._results_fallback_page(
            sport,
