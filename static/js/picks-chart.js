...
function _pcTotalsRow(st, hasProj) {
  ...
  // Prefer data-pl-total / data-xs-total so Totals never shows run-line picks (-1.5).
  function _ouCell(rawTot, scoreline) {
    const raw = String(rawTot || "").trim();
    if (raw && !/[+\-]1\.5\b/.test(raw)) {
      let html = '<span class="proj-total">' + _pcEsc(raw) + "</span>";
      if (scoreline && scoreline !== "—" && !/[+\-]1\.5\b/.test(scoreline)) {
        html +=
          '<span class="proj-scoreline">' + _pcEsc(scoreline) + "</span>";
      }
      return html;
    }
    if (hasProj && scoreline && scoreline !== "—" && !/[+\-]1\.5\b/.test(scoreline)) {
      return _pcFormatProjStackedHtml(scoreline);
    }
    return "—";
  }
  const plScore = hasProj ? _pcPlProjDisplay(st) : "";
  const xsScore = hasProj ? _pcXsProjCell(st).text : "";
  const plProj = _ouCell(st.getAttribute("data-pl-total"), plScore);
  const xsProj = _ouCell(st.getAttribute("data-xs-total"), xsScore);
  return (
    "<tr>" +
    '<td class="mu-col"><span class="mu-away">' +
    away +
...
function _pcActiveDateSection() {
  let sec = document.querySelector(".date-section.visible");
  if (sec) return sec;
  // Race / date-nav edge: chart clicked before showDate, or visible cleared.
  const sections = document.querySelectorAll(".date-section");
  for (let i = 0; i < sections.length; i++) {
    const s = sections[i];
    if (s.querySelector("[data-pick-card], .game-card-stack, .game-card")) {
...
function refreshChartForActive() {
  const sec = _pcActiveDateSection();
  ...
  if (typeof picksViewMode !== "undefined" && picksViewMode === "chart") {
    try {
      ...
      const wrap = _pcEnsureChartWrap(sec);
      if (wrap) {
        wrap.innerHTML =
          '<div style="padding:16px;color:#b91c1c;">Chart failed to render. Try Cards, then Chart again.</div>';
      }
      try {
        console.error("[picks-chart]", err);
      } catch (_e) {}
    }
...
// Override inline template chart (espn_predictions_template) when both load.
window.buildChartTable = buildChartTable;
window.setPicksChartMarket = setPicksChartMarket;
window.setPicksView = setPicksView;
window.refreshChartForActive = refreshChartForActive;