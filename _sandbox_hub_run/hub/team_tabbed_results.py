...
def consensus_models_for_sport(
    ...
    base = custom if custom else tuple(MODEL_ORDER)
    if not finals:
        return base
...
_WNBA_CHART_MODELS = frozenset(_WNBA_CONSENSUS_MODELS) | {"Prediction Lab"}


def filter_wnba_chart_models(payload: dict[str, Any]) -> dict[str, Any]:
    """Drop Grinder2 / Takedown from WNBA chart payload. Do not invent them."""
    if not isinstance(payload, dict):
        return payload

    def _order(names: list[Any] | None) -> list[str]:
        keep = [str(n) for n in (names or []) if str(n) in _WNBA_CHART_MODELS]
        return keep or list(_WNBA_CONSENSUS_MODELS)

    def _models(models: Any) -> Any:
        if not isinstance(models, dict):
            return models
        return {k: v for k, v in models.items() if str(k) in _WNBA_CHART_MODELS}

    out = dict(payload)
    out["model_order"] = _order(out.get("model_order"))
    markets = dict(out.get("markets") or {})
    for key, block in list(markets.items()):
        if not isinstance(block, dict):
            continue
        b = dict(block)
        b["model_order"] = _order(b.get("model_order"))
        tallies = dict(b.get("tallies") or {})
        for wkey, win in list(tallies.items()):
            if not isinstance(win, dict):
                continue
            w = dict(win)
            w["models"] = _models(w.get("models"))
            w["model_order"] = _order(w.get("model_order"))
            tallies[wkey] = w
        b["tallies"] = tallies
        markets[key] = b
    out["markets"] = markets
    return recompute_wnba_ml_from_finals(out)


def _wnba_game_winner(
    g: dict[str, Any],
    ...
    try:
        ...
        hs_i = aa_i = None
    return home, away, hs_i, aa_i, winner


def _wnba_model_correct(g: dict[str, Any], name: str) -> bool | None:
    """Grade one published model pick from the game row. Do not invent a pick."""
    m = (g.get("models") or {}).get(name) or {}
    ...
    if m.get("correct") is False:
        return False
    home, away, hs_i, aa_i, winner = _wnba_game_winner(g)
    grade = _wnba_grade_team_pick(pick, home, away, hs_i, aa_i, winner)
    if grade == "WIN":
        return True
    if grade == "LOSS":
        return False
...
def _wnba_window_label(kind: str, rows: list[dict[str, Any]], ln_key: str) -> str:
    dates = sorted(
        {str(g.get("game_date") or "")[:10] for g in rows if str(g.get("game_date") or "")[:10]}
    )
    graded = sum(
        1
        for g in rows
        if any(_wnba_model_correct(g, n) is not None for n in _WNBA_CONSENSUS_MODELS)
    )
    if kind == "last_night":
        return f"Last Night ({graded} graded · {ln_key})" if ln_key else f"Last Night ({graded} graded)"
    if kind == "last_7":
        if len(dates) >= 2:
            return f"Last 7 ({graded} graded · {dates[0]} → {dates[-1]})"
        if dates:
            return f"Last 7 ({graded} graded · {dates[0]})"
        return f"Last 7 ({graded} graded)"
    return f"Season ({graded} graded)"


def _tally_wnba_window(
    ...
    *,
    ...
    models: dict[str, Any] = {}
    for name in _WNBA_CONSENSUS_MODELS:
        w = l = 0
        for g in rows:
            ok = _wnba_model_correct(g, name)
            if ok is True:
                w += 1
            elif ok is False:
                l += 1
        if w + l:
            models[name] = _wl_block(w, l)
    graded = sum(
        1
        for g in rows
        if any(_wnba_model_correct(g, n) is not None for n in _WNBA_CONSENSUS_MODELS)
    ...
    block: dict[str, Any] = {
        ...
        "games": graded,
        "models": models,
        "model_order": list(_WNBA_CONSENSUS_MODELS),
        "ready": True,
    }
    if date:
        block["date"] = date
    ...
    return _honestize_tally(block)


def _stamp_wnba_face_from_efficiency(g: dict[str, Any]) -> dict[str, Any]:
    """Chart face column is Efficiency for WNBA — same pick the table grades."""
    row = dict(g)
    models = {
        k: v
        for k, v in (row.get("models") or {}).items()
        if str(k) in _WNBA_CHART_MODELS
    }
    row["models"] = models
    eff = models.get("Efficiency") or {}
    if eff.get("pick"):
        row["face_pick"] = eff.get("pick")
        row["face_prob"] = eff.get("prob")
        ok = _wnba_model_correct(row, "Efficiency")
        row["correct"] = ok
    return row


def wnba_period_keys(
    finals: list[dict[str, Any]],
    *,
    last_night_key: str | None = None,
    now: datetime | None = None,
) -> tuple[str, str, str]:
    """Return (last_night_date, last_7_start, today) in America/New_York."""
    clock = now or datetime.now(ZoneInfo("America/New_York"))
    today = clock.strftime("%Y-%m-%d")
    cut7 = (clock.date() - timedelta(days=7)).strftime("%Y-%m-%d")
    past = sorted(
        {
            str(g.get("game_date") or "")[:10]
            for g in finals or []
            if str(g.get("game_date") or "")[:10] and str(g.get("game_date") or "")[:10] < today
        }
    )
    ln_key = last_night_key or (past[-1] if past else (clock.date() - timedelta(days=1)).strftime("%Y-%m-%d"))
    return ln_key, cut7, today


def split_wnba_windows(
    finals: list[dict[str, Any]],
    *,
    last_night_key: str | None = None,
    now: datetime | None = None,
) -> dict[str, list[dict[str, Any]]]:
    rows = [_stamp_wnba_face_from_efficiency(g) for g in _dedupe_finals_by_game(finals)]
    ln_key, cut7, today = wnba_period_keys(rows, last_night_key=last_night_key, now=now)
    ln, d7, season = [], [], []
    for g in rows:
        ...
        if cut7 <= d < today:
            d7.append(g)
    return {"last_night": ln, "last_7": d7, "season": season, "ln_key": ln_key, "cut7": cut7, "today": today}


def consensus_records_by_group(
    finals: list[dict[str, Any]],
    rows: list[dict[str, Any]],
) -> dict[str, dict[str, int]]:
    """W-L per mutually exclusive group_key for a window (4/4, one 3/4, or two 2/4)."""
    wanted = {
        str(g.get("game_id") or "") + "|" + str(g.get("game_date") or "")[:10] + "|"
        + str(g.get("away_team_id") or g.get("away") or "")
        + "@"
        + str(g.get("home_team_id") or g.get("home") or "")
        for g in rows
    }
    out: dict[str, dict[str, int]] = {}
    for a in _wnba_agreements_from_finals(finals):
        key = (
            str(a.get("game_id") or "")
            + "|"
            + str(a.get("game_date") or "")[:10]
            + "|"
            + str(a.get("matchup") or "").replace(" @ ", "@")
        )
        if wanted and key not in wanted and str(a.get("game_date") or "")[:10] not in {
            str(g.get("game_date") or "")[:10] for g in rows
        }:
            continue
        # Match on date+matchup so missing game_id still counts.
        mk = f"{str(a.get('game_date') or '')[:10]}|{a.get('matchup') or ''}"
        row_mks = {
            f"{str(g.get('game_date') or '')[:10]}|"
            f"{g.get('away_team_id') or g.get('away') or ''} @ {g.get('home_team_id') or g.get('home') or ''}"
            for g in rows
        }
        if mk not in row_mks:
            continue
        gk = str(a.get("group_key") or "")
        if not gk:
            continue
        rec = out.setdefault(gk, {"w": 0, "l": 0})
        if a.get("grade") == "WIN":
            rec["w"] += 1
        elif a.get("grade") == "LOSS":
            rec["l"] += 1
    return out


def recompute_wnba_ml_from_finals(
    payload: dict[str, Any],
    *,
    now: datetime | None = None,
) -> dict[str, Any]:
    """Replace moneyline last-night / last-7 / season with counts from the games table."""
    if not isinstance(payload, dict):
        return payload
    markets = dict(payload.get("markets") or {})
    ml = dict(markets.get("moneyline") or {})
    raw = list(ml.get("finals") or payload.get("finals") or [])
    if not raw:
        return payload
    prev_ln = ((ml.get("tallies") or {}).get("last_night") or {})
    windows = split_wnba_windows(
        raw,
        last_night_key=prev_ln.get("date"),
        now=now,
    )
    stamped = windows["season"]
    ln_key = str(windows["ln_key"])
    cut7 = str(windows["cut7"])
    today = str(windows["today"])
    ln_rows, d7_rows = windows["last_night"], windows["last_7"]
    ln_ml = _tally_wnba_window(
        ln_rows,
        label=_wnba_window_label("last_night", ln_rows, ln_key),
        date=ln_key,
    )
    l7_ml = _tally_wnba_window(
        d7_rows,
        label=_wnba_window_label("last_7", d7_rows, ln_key),
        date_from=cut7,
        date_to=today,
    )
    season_ml = _tally_wnba_window(
        stamped,
        label=_wnba_window_label("season", stamped, ln_key),
        date_from=min((str(g.get("game_date") or "")[:10] for g in stamped), default=""),
        date_to=max((str(g.get("game_date") or "")[:10] for g in stamped), default=""),
    )
    ml["finals"] = stamped
    ml["model_order"] = list(_WNBA_CONSENSUS_MODELS)
    ml["tallies"] = {
        "last_night": ln_ml,
        "last_7": l7_ml,
        "season": season_ml,
    }
    ids = [str(f.get("game_id") or "") for f in stamped if f.get("game_id")]
    ml["records_shown"] = len(stamped)
    ml["unique_games"] = len(set(ids)) if ids else len(stamped)
    ml["ungraded"] = sum(1 for f in stamped if f.get("correct") is None)
    markets["moneyline"] = ml
    out = dict(payload)
    out["markets"] = markets
    out["finals"] = stamped
    out["model_order"] = list(_WNBA_CONSENSUS_MODELS)
    out["tallies"] = {
        "last_night": ln_ml,
        "last_7": l7_ml,
        "season": season_ml,
    ...
    try:
        out["analytics"] = build_results_analytics(
            sport="wnba",
            tallies_ml=out["tallies"],
            season_sp=((markets.get("spread") or {}).get("tallies") or {}).get("season") or {},
            season_ou=((markets.get("totals") or {}).get("tallies") or {}).get("season") or {},
            last7_sp=((markets.get("spread") or {}).get("tallies") or {}).get("last_7") or {},
            last7_ou=((markets.get("totals") or {}).get("tallies") or {}).get("last_7") or {},
            finals=stamped,
            snap=None,
        )
    except Exception:
...
def _extract_wnba_model_ml_sides(
    g: dict[str, Any],
) -> tuple[dict[str, str], dict[str, str]] | None:
    """WNBA cards lock 4 models — do not invent Grinder2/Takedown."""
...
def _consensus_agreements_from_finals(
    ...
    *,
    ...
    """ML moneyline agreement rows from model picks (source of truth).

    Even splits are NO_BET (omitted from combo chart). Majorities store
    dissent_models for per-combination rows (MLB UI).
    """
...
_WNBA_MODEL_SHORT = {
    "Edge": "Edge",
    "XSharp": "XSharp",
    "Sharp Consensus": "Sharp Cons.",
    "Efficiency": "Efficiency",
}


def _wnba_ordered_models(names: list[str] | tuple[str, ...]) -> tuple[str, ...]:
    want = {str(n) for n in names}
    return tuple(n for n in _WNBA_CONSENSUS_MODELS if n in want)


def _wnba_group_key(names: list[str] | tuple[str, ...]) -> str:
    return "|".join(_wnba_ordered_models(names))


def _wnba_grade_team_pick(
    pick_team: str,
    home: str,
    away: str,
    hs_i: int | None,
    ...
    if hs_i is None or aa_i is None:
        if not winner:
            return None
        return "WIN" if pick_team.lower() == winner.lower() else "LOSS"
    return _grade_ml_team_pick(pick_team, home, away, hs_i, aa_i)


def _wnba_agreements_from_finals(finals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """One graded row per model group: 4/4, each 3/4 trio, each 2/4 pair."""
    out: list[dict[str, Any]] = []
    for g in _dedupe_finals_by_game(finals):
        extracted = _extract_wnba_model_ml_sides(g)
        if not extracted:
            continue
        sides, _picks = extracted
        home = str(g.get("home_team_id") or g.get("home") or g.get("home_team") or "")
        ...
        try:
            ...
            hs_i = aa_i = None
        by_side: dict[str, list[str]] = {"HOME": [], "AWAY": []}
        for name in _WNBA_CONSENSUS_MODELS:
            side = sides.get(name)
            if side in by_side:
                by_side[side].append(name)
        groups: list[tuple[int, list[str], str]] = []
        home_n, away_n = len(by_side["HOME"]), len(by_side["AWAY"])
        if home_n == 4:
            groups.append((4, list(_WNBA_CONSENSUS_MODELS), home))
        elif away_n == 4:
            groups.append((4, list(_WNBA_CONSENSUS_MODELS), away))
        elif home_n == 3:
            groups.append((3, by_side["HOME"], home))
        elif away_n == 3:
            groups.append((3, by_side["AWAY"], away))
        elif home_n == 2 and away_n == 2:
            groups.append((2, by_side["HOME"], home))
            groups.append((2, by_side["AWAY"], away))
        dk = str(g.get("game_date") or "")[:10]
        for agree_n, names, pick_team in groups:
            grade = _wnba_grade_team_pick(
                pick_team, home, away, hs_i, aa_i, winner
            )
            if grade is None:
                continue
            ordered = _wnba_ordered_models(names)
            out.append(
                {
                    "agree_n": agree_n,
                    "is_unanimous": agree_n == 4,
                    "is_three_three": agree_n == 2,
                    "grade": grade,
                    "game_date": dk,
                    "game_id": str(g.get("game_id") or ""),
                    "matchup": f"{away} @ {home}",
                    "unanimous_pick": pick_team if agree_n == 4 else "",
                    "graded_pick": pick_team,
                    "graded_models": list(ordered),
                    "group_key": _wnba_group_key(ordered),
                }
            )
    return out


def _wnba_group_label_html(agree_n: int, models: tuple[str, ...]) -> str:
    shown = " · ".join(_WNBA_MODEL_SHORT.get(n, n) for n in models)
    if agree_n >= 4:
        return (
            "4/4 unanimous"
            f"<div class='cons-split-note'>All four: {html.escape(shown)}</div>"
        )
    if agree_n == 3:
        dissent = [n for n in _WNBA_CONSENSUS_MODELS if n not in models]
        d = _WNBA_MODEL_SHORT.get(dissent[0], dissent[0]) if dissent else "—"
        return (
            "3/4"
            f"<div class='cons-split-note'>{html.escape(shown)}"
            f"<br>Dissent: {html.escape(d)}</div>"
        )
    return (
        "2/4"
        f"<div class='cons-split-note'>{html.escape(shown)}</div>"
    )


def build_wnba_consensus_records_html(
    finals: list[dict[str, Any]],
    *,
    last_night_key: str | None = None,
) -> str:
    """Consensus table: 4/4, then each 3/4 trio, then each 2/4 pair."""
    agreements = _wnba_agreements_from_finals(_dedupe_finals_by_game(finals))
    if not agreements:
        return ""
    ...
    def by_group(items: list[dict[str, Any]]) -> dict[str, list]:
        buckets: dict[str, list] = {}
        for a in items:
            key = str(a.get("group_key") or "")
            if not key:
                continue
            buckets.setdefault(key, []).append(a)
        return buckets

    ln_b, d7_b, d30_b = by_group(ln), by_group(d7), by_group(d30)
    keys = sorted(
        {a.get("group_key") for a in agreements if a.get("group_key")},
        key=lambda k: (
            -len(str(k).split("|")),
            tuple(
                _WNBA_CONSENSUS_MODELS.index(n)
                if n in _WNBA_CONSENSUS_MODELS
                else 99
                for n in str(k).split("|")
            ),
        ),
    )
    rows_html = []
    for key in keys:
        names = tuple(str(key).split("|"))
        agree_n = len(names)
        label = _wnba_group_label_html(agree_n, names)
        rows_html.append(
            "<tr>"
            f'<td class="bucket">{label}</td>'
            f"<td>{_consensus_record_cell(ln_b.get(key, []), bar=True, empty='—')}</td>"
            f"<td>{_consensus_record_cell(d7_b.get(key, []), bar=True, empty='—')}</td>"
            f"<td>{_consensus_record_cell(d30_b.get(key, []), bar=True, empty='—')}</td>"
            "</tr>"
        )
    ...
    <div class="pl-consensus-records" id="pl-consensus-records">
      ...
      <p class="sub">Agreement among Edge, XSharp, Sharp Consensus, and Efficiency. 4/4 is all four. Each 3/4 row is a specific trio (the fourth dissented). Each 2/4 row is one pair’s moneyline — the other pair is the next 2/4 row, graded on its own pick.</p>
      ...
      <style>
        .pl-consensus-records,.cfl-consensus{{background:#fff;border:1px solid rgba(15,23,42,.12);border-radius:14px;padding:18px;margin:16px 0 20px;max-width:none;width:100%;box-sizing:border-box}}
        ...
        .pl-consensus-records .cons-split-note{{margin-top:4px;font-size:.72rem;font-weight:500;color:#64748b;line-height:1.35;text-align:left}}
...
def _consensus_bucket_label_html(label: str, *, graded_models: list[str] | None = None) -> str:
    """Row label; 3/6 split lists the three models on the graded (winning) side — no ⓘ."""
    if label != "3/6 split":
        return label
    models = [str(m) for m in (graded_models or []) if m]
    if models:
        note = "Graded models: " + html.escape(", ".join(models))
    else:
        note = "Graded models: the three on the winning side"
    return (
        "3/6 split"
        f"<div class='cons-split-note'>{note}</div>"
    )
...
def build_run_line_model_consensus_30d_html(
    ...
    """30-day run-line W-L/% by six-model moneyline agreement (no new market tab).

    Buckets use the same ML agreement counts as moneyline consensus. Each game is
    graded on the published Prediction Lab run-line pick (−1.5) when present.
    """
    rows = _dedupe_finals_by_game(finals)
    if not rows:
        return ""
    ...
    buckets: dict[str, list[dict[str, Any]]] = {b: [] for b in labels}

    for g in rows:
        dk = str(g.get("game_date") or "")[:10]
        if not dk or not (cut30 <= dk < today):
            continue
        ...
        sides, _picks = extracted
        models = g.get("models") or {}
        agree_n, is_unanimous, _maj, _split = _ml_agreement_counts(sides, models)
        label = _consensus_bucket_label_smart(agree_n)
        if label in ("6/6 unanimous", "6/6 agree") and not is_unanimous:
            continue
        blk = g.get("spread") if isinstance(g.get("spread"), dict) else {}
        grade = blk.get("grade")
        if grade not in ("WIN", "LOSS"):
            # Fallback: grade PL pick from score when attrs present
            home = str(g.get("home_team_id") or g.get("home") or g.get("home_team") or "")
            away = str(g.get("away_team_id") or g.get("away") or g.get("away_team") or "")
            pl_pick = str(blk.get("pl_pick") or "").strip()
            try:
                hs = int(g.get("home_score"))
                aa = int(g.get("away_score"))
            except (TypeError, ValueError):
                continue
            if not pl_pick or not home or not away:
                continue
            try:
                from mlb_three_way_consensus import _grade_spread, _spread_side_token

                side = _spread_side_token(pl_pick, home, away)
                grade = _grade_spread(side, home, away, hs, aa) if side else ""
            except Exception:
                grade = ""
            if grade not in ("WIN", "LOSS"):
                continue
        buckets.setdefault(label, []).append({"grade": grade, "game_date": dk})

    if not any(buckets.get(lab) for lab in labels):
        return ""

    rows_html = []
    for label in labels:
        items = buckets.get(label) or []
        empty = "0-0" if label == "3/6 split" else "—"
        rows_html.append(
            "<tr>"
            f'<td class="bucket">{_consensus_bucket_label_html(label)}</td>'
            f"<td>{_consensus_record_cell(items, bar=True, empty=empty)}</td>"
            "</tr>"
        )
    return f"""
    <div class="pl-consensus-records pl-rl-ml-consensus-30d" id="pl-rl-ml-consensus-30d">
      <h2>Run Line · Model Consensus (30 days)</h2>
      <p class="sub">Moneyline model agreement buckets, graded on the published run-line pick (−1.5).</p>
      <div style="overflow-x:auto">
        <table>
          <thead>
            <tr>
      ...
      <style>
        .pl-rl-ml-consensus-30d{{background:#fff;border:1px solid rgba(15,23,42,.12);border-radius:14px;padding:18px;margin:16px auto 20px;max-width:1100px}}
        .pl-rl-ml-consensus-30d h2{{margin:0 0 6px;font-size:1.15rem;color:#0f172a;text-align:center}}
        .pl-rl-ml-consensus-30d .sub{{margin:0 0 14px;color:#64748b;font-size:.88rem;text-align:center}}
        .pl-rl-ml-consensus-30d table{{width:100%;border-collapse:collapse;font-size:.9rem}}
        .pl-rl-ml-consensus-30d th,.pl-rl-ml-consensus-30d td{{padding:10px 8px;border-bottom:1px solid #e2e8f0;text-align:center}}
        .pl-rl-ml-consensus-30d th{{font-size:.72rem;text-transform:uppercase;letter-spacing:.04em;color:#64748b}}
        .pl-rl-ml-consensus-30d td.bucket{{text-align:left;font-weight:700;color:#0f172a}}
      </style>
    </div>
    """


def _build_combo_consensus_records_html(
    finals: list[dict[str, Any]],
    *,
    last_night_key: str | None = None,
    sport: str = "",
    ...
    """New consensus table: one row per dissent combination (MLB live format)."""
    sport_l = (sport or "").strip().lower() or "mlb"
    deduped = _dedupe_finals_by_game(finals)
    model_order = consensus_models_for_sport(sport_l, deduped)
    panel_n = int(panel_n or len(model_order) or 6)
    min_majority = panel_n // 2 + 1
    folded_levels = tuple(range(panel_n, min_majority - 1, -1))
    agreements = _consensus_agreements_from_finals(deduped, sport=sport_l)
    if not agreements:
        return ""
    now = datetime.now(ZoneInfo("America/New_York"))
    yesterday = (now - timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    def dissent_key(a: dict[str, Any]) -> tuple[str, ...]:
        ...
        if present:
            return tuple(name for name in model_order if name in present)
        maj = {str(m) for m in (a.get("majority_models") or a.get("graded_models") or []) if m}
        return tuple(name for name in model_order if name not in maj)
    ...
    for folded in folded_levels:
        ...
        for key in keys:
            rows_html.append(
                ...
                f"</td>"
                f"<td>{_consensus_record_cell(filter_combo(ln, folded=folded, key=key), bar=True, empty=empty)}</td>"
                f"<td>{_consensus_record_cell(filter_combo(d7, folded=folded, key=key), bar=True, empty=empty)}</td>"
                f"<td>{_consensus_record_cell(filter_combo(d30, folded=folded, key=key), bar=True, empty=empty)}</td>"
                "</tr>"
            )
...
def build_consensus_records_html(
    ...
    *,
    ...
    """HTML for Consensus Based Betting Records. Empty string if no model data."""
    sport_l = (sport or "").strip().lower()
    # MLB-format dissent combinations (not coarse 6/5/4/3 buckets).
    if sport_l in ("tennis", "ufc"):
        return _build_combo_consensus_records_html(
            finals, last_night_key=last_night_key, sport=sport_l
        )
    agreements = _consensus_agreements_from_finals(
        _dedupe_finals_by_game(finals), sport=sport_l or "mlb"
    )
    if not agreements:
        return ""
    smart = True
    ...
    # Prefer last-night 3/6 models for the row label; else any recent 3/6 game.
    split_models: list[str] = []
    for pool in (ln_b.get("3/6 split") or [], d7_b.get("3/6 split") or [], d30_b.get("3/6 split") or []):
        for a in pool:
            mods = [str(m) for m in (a.get("graded_models") or []) if m]
            if len(mods) == 3:
                split_models = mods
                break
        if split_models:
    ...
    rows_html = []
    for label in labels:
        models_arg = split_models if label == "3/6 split" else None
        # 3/6 with no games in a window: show 0-0 (not —). Matches totals three-way
        # copy and the checker that requires a Last-night W-L on this row.
        empty = "0-0" if label == "3/6 split" else "—"
        rows_html.append(
            "<tr>"
            f'<td class="bucket">{_consensus_bucket_label_html(label, graded_models=models_arg)}</td>'
            f"<td>{_consensus_record_cell(ln_b.get(label, []), bar=smart, empty=empty)}</td>"
            f"<td>{_consensus_record_cell(d7_b.get(label, []), bar=smart, empty=empty)}</td>"
            f"<td>{_consensus_record_cell(d30_b.get(label, []), bar=smart, empty=empty)}</td>"
            "</tr>"
        )
    ln_hdr = f"Last night ({ln_key})" if ln_key else "Last night"
...
def _plxs_calibration_line(
    d30: dict[str, list], *, market: str, sport: str = "mlb"
) -> str:
    ...
    *,
    ...
    if market == "spread":
        if sport_l == "mlb":
            ...
            line = "puck line"
            title = "Prediction Lab & XSharp — Puck Line"
            sub = (
                "Prediction Lab is the published puck-line pick. XSharp is that "
                "model's pre-game puck line. Same last-night, past-7, and past-30 "
                "windows as moneyline."
            )
        else:
            line = "spread"
            title = "Prediction Lab & XSharp — Spread"
            sub = (
                "Prediction Lab is the published spread pick. XSharp is that "
                "model's pre-game spread. Same last-night, past-7, and past-30 "
                "windows as moneyline."
            )
    else:
...
def _wrap_results_markets(
    ...
    *,
    ...
    for key, label in (
        ...
        tabs.append(
            f'<a class="{cls}" href="{href}" data-pl-market="{key}">{label}</a>'
        )
    panel_html = []
    ...
    width_css = (
        ".pl-results-markets{max-width:none;width:100%;margin:16px 0 20px;box-sizing:border-box}"
        ".pl-results-markets .pl-consensus-records{max-width:none!important;width:100%;margin:0}"
        if sport_l == "wnba"
        else (
    ...
    <div class="pl-results-markets" id="pl-results-markets">
      ...
      (function(){{
        ...
        if(!root||root.getAttribute("data-pl-market-js")) return;
        root.setAttribute("data-pl-market-js","1");
        root.addEventListener("click",function(e){{
          var a=e.target&&e.target.closest?e.target.closest("a[data-pl-market]"):null;
          if(!a||!root.contains(a)) return;
          e.preventDefault();
          var key=a.getAttribute("data-pl-market");
          root.querySelectorAll("a[data-pl-market]").forEach(function(t){{
            t.classList.toggle("active", t.getAttribute("data-pl-market")===key);
          }});
          root.querySelectorAll("[data-market-panel]").forEach(function(p){{
            if(p.getAttribute("data-market-panel")===key) p.removeAttribute("hidden");
...
def inject_consensus_records_html(
    ...
    *,
    ...
    if sport_l == "wnba":
        ...
        block = (
            build_wnba_consensus_records_html(
                rows, last_night_key=last_night_key
            )
            if rows
            else ""
        )
    ...
    if sport_l == "mlb":
        try:
            rl30 = build_run_line_model_consensus_30d_html(rows or [])
        except Exception as e:
            print(f"[hub] RL ML consensus 30d: {e}", flush=True)
            rl30 = ""
    combined = (block or "") + (rl30 or "")
    if not combined:
        return html
    if sport_l == "wnba" and chart_view:
...
def _insert_wnba_chart_consensus(html: str, block: str) -> str:
    """Consensus immediately above the moneyline games / cards."""
    if not html or not block:
        return html
    for pat in (
        r'(<section\b[^>]*\bid=["\']ssr-finals["\'][^>]*>)',
        r'(<h2\b[^>]*>\s*Moneyline games)',
        r'(<section\b[^>]*\bid=["\']finals-wrap["\'][^>]*>)',
        r'(<div class="date-section\b)',
        r'(<div class=["\']games-grid["\'])',
        r'(<div class="game-card-stack\b)',
    ):
...
def _insert_consensus_below_season(html: str, block: str) -> str:
    """Consensus sits under the chart games table (not above it).

    Actual game rows must stay visible right under the tallies; burying them
    under consensus made chart view look like the games were missing.
    """
    if not html or not block:
        return html
    ...
    if slot:
        at = slot.end()
        return html[:at] + block + html[at:]
    # Chart: after CSR games section (preferred) or SSR games table.
    m = re.search(
        r'<section\b[^>]*\bid=["\']finals-wrap["\'][\s\S]*?</section>',
        html,
        flags=re.I,
    )
    if m:
...
def markets_from_live_html(html: str, sport: str) -> dict[str, Any]:
    ...
    if (sport or "").lower() == "mlb":
        ...
        # WNBA: never invent Grinder2/Takedown or fill missing Edge/XSharp from face.
        if (sport or "").lower() == "wnba":
            finals = rows
        else:
            finals = synthesize_missing_ml_models(rows)
...
def inject_ssr_chart_bootstrap(html: str, payload: dict[str, Any], sport: str) -> str:
    ...
    _sport_l = str(sport or "").lower()
    _finals_cap = 800 if _sport_l in ("cfl", "wnba") else 40
    ...
    if str(sport or "").lower() == "wnba":
        order = [n for n in order if n in _WNBA_CHART_MODELS] or list(
            _WNBA_CONSENSUS_MODELS
        )
    ...
    ssr_finals = (
        ...
        f"<th>{'Efficiency pick' if _sport_l == 'wnba' else 'Edge pick'}</th>"
        "<th>%</th><th>Result</th><th>Models</th></tr></thead>"
        f"<tbody>{''.join(rows) or '<tr><td colspan=8 class=muted>No finals.</td></tr>'}</tbody>"
        "</table></div></section>"
    )
    bootstrap = tallies_bootstrap + ssr_finals

    # Fill #tallies (windows only) and place games immediately after it.
    if re.search(r'id=["\']tallies["\']', html, flags=re.I):
        html = re.sub(
            r'(<div\b[^>]*\bid=["\']tallies["\'][^>]*)\s*hidden([^>]*>)\s*</div>',
            r'\1\2' + tallies_bootstrap + "</div>" + ssr_finals,
            html,
            count=1,
            flags=re.I,
        )
    ...
    already_bootstrapped = (
        'id="ssr-finals"' in html
        or "id='ssr-finals'" in html
        or tallies_bootstrap in html
    )
    _sport_l = str(sport or "").lower()
    # Never dump a second Best Performing / games copy before the footer when
    # #tallies was already filled (UFC/tennis/WNBA were double-rendering).
    ...
    if _sport_l in ("wnba", "ufc", "tennis"):
        html = _strip_wnba_orphan_chart_blocks(html)
...
def _strip_wnba_orphan_chart_blocks(html: str) -> str:
    """Drop leftover tally/games blocks that landed between </main> and <footer>."""
    if not html:
        return html
    m = re.search(r"</main>([\s\S]*?)(<footer\b)", html, flags=re.I)
    if not m:
        return html
    mid = m.group(1)
    if not re.search(r"ssr-finals|class=\"tally\"|pl-analytics", mid, flags=re.I):
        return html
