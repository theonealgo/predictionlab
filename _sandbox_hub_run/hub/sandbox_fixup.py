...
# MLB UI SIGNED OFF — do not change without owner request (locked 2026-08-10).
...
MLB_FLIP_SPREAD = os.environ.get("MLB_FLIP_SPREAD", "0").strip().lower() not in (
    "0",
    "false",
    "off",
    "no",
...
def staging_full_site() -> bool:
    """When true, keep live site header/nav from work2 — only patch card/chart content."""
    return os.environ.get("STAGING_FULL_SITE", "1").strip().lower() not in (
        "0",
        "false",
        "off",
        "no",
...
def _ensure_sandbox_sport_attr(html: str, sport: str) -> str:
    """Stamp data-sandbox-sport on <body> so MLB grid/CSS fixups can target the page."""
    if not html or not sport:
        return html
    sport = sport.lower()
    if re.search(rf'data-sandbox-sport=["\']{re.escape(sport)}["\']', html, re.I):
        return html
    if re.search(r"\bdata-sandbox-sport=", html, re.I):
        return re.sub(
            r'\bdata-sandbox-sport=["\'][^"\']*["\']',
            f'data-sandbox-sport="{sport}"',
            html,
            count=1,
            flags=re.I,
        )
    return re.sub(
        r"<body\b([^>]*)>",
        rf'<body\1 data-sandbox-sport="{sport}">',
        html,
        count=1,
        flags=re.I,
    )
...
def _dedupe_game_card_stacks_chunk(html: str) -> str:
    """Keep first game-card-stack per away+home+kickoff inside one chunk."""
    if not html or "game-card-stack" not in html:
        return html
    parts: list[str] = []
    pos = 0
...
def dedupe_game_card_stacks(html: str) -> str:
    """Drop identical duplicate stacks within each date tab (not across days)."""
    if not html or "game-card-stack" not in html:
        return html
    if 'class="date-section' not in html:
        return _dedupe_game_card_stacks_chunk(html)
    parts = re.split(r'(?=<div class="date-section\b)', html, flags=re.I)
    if len(parts) <= 1:
        return _dedupe_game_card_stacks_chunk(html)
    return parts[0] + "".join(_dedupe_game_card_stacks_chunk(p) for p in parts[1:])
...
def ensure_toggle_pick_details_js(html: str) -> str:
    ...
    if not html or "view-details-btn" not in html:
        return html
    html = _open_all_pick_details_html(html)
...
function _sandboxLabelDetailsBtn(btn, open){
  if(!btn) return;
  var chev=btn.querySelector('.chevron');
  var chevHtml=chev?chev.outerHTML:' <span class="chevron">▾</span>';
  btn.innerHTML=(open?'Less details':'View Details')+' '+chevHtml;
}
function _sandboxForcePickDetails(btn, open){
  ...
  _sandboxLabelDetailsBtn(btn, open);
}
...
function _sandboxOpenAllPickDetails(){
  document.querySelectorAll('.view-details-btn').forEach(function(btn){
    _sandboxForcePickDetails(btn, true);
  });
}
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded', _sandboxOpenAllPickDetails);
} else {
  _sandboxOpenAllPickDetails();
}
setTimeout(_sandboxOpenAllPickDetails, 50);
setTimeout(_sandboxOpenAllPickDetails, 400);
...
def _open_all_pick_details_html(html: str) -> str:
    """Server-side: expand all MLB card details (do not rely on JS alone)."""
    if not html or "card-details" not in html:
        return html
    html = re.sub(
        r'(<div\b[^>]*\bclass="[^"]*\bcard-details\b[^"]*"[^>]*)\s*\bhidden\b([^>]*>)',
        r"\1\2",
        html,
        flags=re.I,
    )
    ...
    html = re.sub(
        r'(<(?:button|a)\b[^>]*\bview-details-btn\b[^>]*>)\s*View Details\s*',
        r"\1Less details ",
        html,
        flags=re.I,
    )

    def _expand_stack(m: re.Match[str]) -> str:
        tag = m.group(0)
    ...
    # Only expand outer stacks — marking every nested pick-card breaks the 3-col grid.
    html = re.sub(
...
def enrich_mlb_chart_data_attrs(html: str) -> str:
    ...
    def _patch_stack(stack: str) -> str:
        ...
        open2 = open_tag
        # Pick Confidence pc-boxes → chart model columns (home-win % on pick side).
        home_m = re.search(r'\bdata-home="([^"]*)"', open_tag, flags=re.I)
        away_m = re.search(r'\bdata-away="([^"]*)"', open_tag, flags=re.I)
        home_n = (home_m.group(1) if home_m else "").strip()
        away_n = (away_m.group(1) if away_m else "").strip()
...
def inject_picks_chart_tabs(
    ...
    *,
    ...
    if "picks-chart.js" in html and "PICKS_CHART" in html:
        # Still relocate controls; then fall through to append a fresh override script.
        html = _relocate_picks_chart_controls_below_header(html)
    else:
    ...
    # pc19+: always load last so hub Totals row wins over inline template chart.
    assets_sync = (
        f"{note}"
...
def strip_mlb_picks_chart_total_ev(html: str) -> str:
    """Remove Total EV column from MLB picks chart (inline sidecar JS or picks-chart.js)."""
    if not html or "picksChartMarket" not in html:
        return html
    html = re.sub(
        r"\+_infoTh\('Total EV',\s*TOTAL_EV_TIP\);",
        "",
        html,
        flags=re.I,
    )
    html = re.sub(
        r"\+_pcInfoTh\(\"Total EV\",\s*TOTAL_EV_TIP\);",
        "",
        html,
        flags=re.I,
    )
...
def rewrite_mlb_edge_chip_to_consensus(html: str) -> str:
    ...
    def _agree_n(stack: str) -> int | None:
        ...
        # Fallback: data-m-* vs published pick
        ...
        pick_home = bool(home and pick and pick in home)
        if not pick_home and not (away and pick and pick in away):
            return None
        keys = (
            "data-m-grinder2",
            "data-m-takedown",
        ...
        for k in keys:
            ...
            if v <= 1.5:
                v *= 100.0
            found += 1
            # Attrs are pick-side win%; treat >=50 as agreeing with published pick.
            if v >= 50.0:
                n += 1
...
def ensure_picks_chart_six_model_columns(html: str) -> str:
    """Live inline buildChartTable drops model columns when no card has that data-* attr.

    Sandbox always shows all six MLB model columns (N/A when missing). picks-chart.js
    already does this; patch the inline duplicate so it cannot win if it loads last.
    """
    if not html or "ALL_MODEL_ATTRS.forEach" not in html:
        return html
    patched = re.sub(
        r"const\s+MODEL_ATTRS\s*=\s*\[\s*\];\s*"
        r"const\s+MODEL_LABELS\s*=\s*\[\s*\];\s*"
        r"ALL_MODEL_ATTRS\.forEach\s*\(\s*function\s*\(\s*a\s*,\s*i\s*\)\s*\{"
        r"[\s\S]*?"
        r"if\s*\(\s*any\s*\)\s*\{\s*MODEL_ATTRS\.push\(a\);\s*MODEL_LABELS\.push\(ALL_MODEL_LABELS\[i\]\);\s*\}\s*"
        r"\}\s*\)\s*;",
        "const MODEL_ATTRS = ALL_MODEL_ATTRS.slice();\n"
        "        const MODEL_LABELS = ALL_MODEL_LABELS.slice();",
        html,
        count=1,
        flags=re.I,
    )
    return patched if patched != html else html


def ensure_mlb_pick_conf_no_scroll(html: str) -> str:
    """MLB PICKS ONLY (LOCKED): 3 cards/row; pick-conf stays 6-up inside each card.

    Do not change this for results work. Results use ensure_mlb_results_card_layout.
    Chart mode must beat the cards grid !important rules or Chart looks blank.
    """
    if not html:
        return html
    # Always refresh this block so chart-mode CSS stays current.
    html = re.sub(
        r'<style id="sandbox-mlb-pick-conf-no-scroll">[\s\S]*?</style>',
        "",
        html,
        flags=re.I,
    )
    css = (
        '<style id="sandbox-mlb-pick-conf-no-scroll">'
        "html body[data-sandbox-sport=\"mlb\"]{"
        "--pl-card-min:320px!important;--pl-card-max:none!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .pick-conf-bar,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .pick-conf-bar{"
        "overflow-x:hidden!important;overflow-y:visible!important;max-width:100%!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .pick-conf-grid,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .pick-conf-grid{"
        "display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;"
        "gap:4px!important;min-width:0!important;width:100%!important;max-width:100%!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .pc-box,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .pc-box{"
        "min-width:0!important;width:100%!important;box-sizing:border-box!important;}"
        # Cards grid only outside Chart mode (else !important fights chart hide).
        "html body[data-sandbox-sport=\"mlb\"] .date-section:not(.chart-mode) .games-grid,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .date-section:not(.chart-mode) .games-grid{"
        "display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;"
        "gap:16px!important;align-items:start!important;width:100%!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .date-section:not(.chart-mode) .games-grid > .game-card-stack,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .date-section:not(.chart-mode) .games-grid > .game-card-stack{"
        ...
        "html body[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .games-grid,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .games-grid,"
        "html body[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .game-card-stack,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .game-card-stack{"
        "display:none!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .chart-table-wrap,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .date-section.chart-mode .chart-table-wrap{"
...
def _shorten_mlb_results_pc_sides(html: str) -> str:
    """Results cards: keep model % + mark; drop long team names that mash in 6-up."""
...
def ensure_mlb_results_card_layout(html: str) -> str:
    """MLB results only: 3 game cards/row; pick-conf 3×2 readable; tallies 3-col."""
    if not html:
        return html
    html = _shorten_mlb_results_pc_sides(html)
    if 'id="sandbox-mlb-results-card-layout"' in html:
        return html
    css = (
        '<style id="sandbox-mlb-results-card-layout">'
        "html body[data-sandbox-sport=\"mlb\"]{"
        "--pl-card-min:320px!important;--pl-card-max:none!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .games-grid,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .games-grid,"
        "html body[data-sandbox-sport=\"mlb\"] .results-grid{"
        "display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;"
        "gap:16px!important;align-items:start!important;width:100%!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .games-grid > .game-card,"
        "html body.research-site[data-sandbox-sport=\"mlb\"] .games-grid > .game-card,"
        "html body[data-sandbox-sport=\"mlb\"] .games-grid > .game-card-stack{"
        ...
        "gap:4px!important;padding:8px 6px!important;min-height:72px!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .pc-name{"
        "font-size:0.62rem!important;line-height:1.15!important;white-space:nowrap!important;"
        ...
        "html body[data-sandbox-sport=\"mlb\"] .pc-val{"
        "font-size:0.95rem!important;font-weight:800!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .pc-side{"
        "font-size:0.85rem!important;white-space:nowrap!important;overflow:hidden!important;"
        "max-width:100%!important;min-height:0!important;padding:2px 4px!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .daily-tally-grid{"
        "display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;"
        "gap:10px!important;}"
        "html body[data-sandbox-sport=\"mlb\"] .daily-tally-card{"
        "min-width:0!important;width:100%!important;box-sizing:border-box!important;}"
        "@media(max-width:1100px){"
        "html body[data-sandbox-sport=\"mlb\"] .games-grid{"
        "grid-template-columns:repeat(2,minmax(0,1fr))!important;}"
...
def ensure_wnba_pick_card_layout(html: str) -> str:
    """WNBA picks: 3 cards/row, same spacing as locked MLB. Does not touch MLB CSS."""
    if not html:
        return html
    if 'id="sandbox-wnba-pick-card-layout"' in html:
        return html
    css = (
        '<style id="sandbox-wnba-pick-card-layout">'
        "html body[data-sandbox-sport=\"wnba\"]{"
        "--pl-card-min:320px!important;--pl-card-max:none!important;}"
        "html body[data-sandbox-sport=\"wnba\"] .date-section:not(.chart-mode) .games-grid,"
        "html body.research-site[data-sandbox-sport=\"wnba\"] .date-section:not(.chart-mode) .games-grid,"
        "html body[data-sandbox-sport=\"wnba\"] .games-grid{"
        "display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;"
        "gap:16px!important;align-items:start!important;width:100%!important;}"
        "html body[data-sandbox-sport=\"wnba\"] .date-section:not(.chart-mode) .games-grid > .game-card-stack,"
        "html body.research-site[data-sandbox-sport=\"wnba\"] .date-section:not(.chart-mode) .games-grid > .game-card-stack,"
        "html body[data-sandbox-sport=\"wnba\"] .games-grid > .game-card-stack{"
        "display:block!important;max-width:none!important;width:100%!important;"
        "min-width:0!important;margin:0!important;box-sizing:border-box!important;"
        "justify-self:stretch!important;}"
        "@media(max-width:1100px){"
...
def hide_wnba_grinder_takedown(html: str) -> str:
    """WNBA does not publish Grinder2 / Takedown — drop those tiles, do not fill them."""
    if not html:
        return html
    if "Grinder2" not in html and "Takedown" not in html:
        return html

    def _drop_named_divs(src: str, class_pat: str) -> str:
        out: list[str] = []
        pos = 0
        cre = re.compile(rf'<div\b[^>]*\bclass="[^"]*\b{class_pat}\b[^"]*"[^>]*>', re.I)
        while True:
            m = cre.search(src, pos)
            if not m:
                out.append(src[pos:])
                break
            box, end = _balanced_div_at(src, m.start())
            if end < 0:
                out.append(src[pos:])
                break
            out.append(src[pos : m.start()])
            head = box[:800]
            if re.search(r"Grinder2|Takedown", head, re.I):
                pos = end
                continue
            out.append(box)
            pos = end
    ...
    html = _drop_named_divs(html, "daily-tally-card")
    html = _drop_named_divs(html, "tally-card")
    html = _drop_named_divs(html, "model-card")
    html = _drop_named_divs(html, "pc-box")
    html = re.sub(
        r"<th\b[^>]*>\s*(?:⭐\s*)?(?:Grinder2|Takedown)\s*</th>",
        "",
        html,
        flags=re.I,
    )
...
def ensure_wnba_results_layout(html: str) -> str:
    """WNBA only: full-width consensus + tally rows. Does not touch MLB."""
    if not html:
        return html
    html = _ensure_sandbox_sport_attr(html, "wnba")
    if 'id="sandbox-wnba-results-layout"' in html:
        return html
    css = (
        '<style id="sandbox-wnba-results-layout">'
        "body[data-sandbox-sport=\"wnba\"] section.pl-analytics,"
        "body[data-sandbox-sport=\"wnba\"] .pl-mlb-analytics,"
        "body[data-sandbox-sport=\"wnba\"] .pl-results-markets,"
        "body[data-sandbox-sport=\"wnba\"] .pl-consensus-records,"
        "body[data-sandbox-sport=\"wnba\"] .tally-wrap,"
        "body[data-sandbox-sport=\"wnba\"] #ssr-finals,"
        "body[data-sandbox-sport=\"wnba\"] #finals-wrap{"
        "max-width:none!important;width:100%!important;"
        "margin-left:0!important;margin-right:0!important;box-sizing:border-box;}"
        "body[data-sandbox-sport=\"wnba\"] .tally-wrap > .tally,"
        "body[data-sandbox-sport=\"wnba\"] .tally-wrap > section.pl-analytics{"
        "width:100%!important;max-width:none!important;"
        "margin-left:0!important;margin-right:0!important;justify-self:stretch;}"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] section.pl-analytics,"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] .pl-mlb-analytics{"
        "display:none!important;}"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] > section.tally,"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] > section#ssr-finals{"
        "display:none!important;}"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] .pl-results-market-tabs{"
        "display:none!important;}"
        "body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] .tally-wrap .tally-grid{"
        "justify-content:center;}"
        "</style>"
        "<script>"
        "(function(){"
        "function kill(){"
        "document.querySelectorAll('body[data-sandbox-sport=\"wnba\"][data-ssr-chart=\"1\"] section.pl-analytics').forEach(function(el){el.remove();});"
        "}"
        "function sync(key){"
        ...
        "setTimeout(kill,0);setTimeout(kill,400);"
        "var tabs=document.getElementById('market-tabs');"
        "if(tabs){"
        "tabs.addEventListener('click',function(e){"
        "var btn=e.target&&e.target.closest&&e.target.closest('[data-market]');"
        "if(btn)sync(btn.getAttribute('data-market'));"
        "});"
        "}"
        "})();"
        "</script>"
...
def apply_wnba_ui_fixups(html: str, which: str = "picks") -> str:
    """Unlock + open details + 3-col picks grid. No chrome swap."""
    if not html:
        return html
    html = unlock_premium_card_details(html)
    html = _ensure_sandbox_sport_attr(html, "wnba")
    html = hide_wnba_grinder_takedown(html)
    html = _open_all_pick_details_html(html)
    html = ensure_toggle_pick_details_js(html)
    if (which or "picks").strip().lower() == "picks":
        html = ensure_wnba_pick_card_layout(html)
    if (which or "").strip().lower() == "results":
        html = ensure_wnba_results_layout(html)
    return html
...
def apply_sport_fixups(html: str, sport: str, which: str = "picks") -> str:
    ...
    # Full-site mode: keep live header/nav — still unlock cards + MLB content fixups.
    # Do NOT swap chrome (ensure_canonical_chrome) or re-flip spreads already applied upstream.
    if staging_full_site() and sport in ("mlb", "soccer", "wnba"):
        html = unlock_premium_card_details(html)
        html = strip_sandbox_dev_notes(html)
        html = strip_sport_subnav(html)
        html = _ensure_sandbox_sport_attr(html, sport)
        if sport == "mlb" and which == "picks":
            if "<!-- mlb-ui-fixup -->" not in html:
            ...
            # Always inject hub picks-chart.js so Totals overrides inline template chart
            # (inline setPicksChartMarket alone left Totals showing run-line cells).
            html = inject_mlb_picks_chart_tabs(html)
            html = strip_mlb_picks_chart_total_ev(html)
            html = ensure_mlb_pick_conf_no_scroll(html)
            html = ensure_picks_chart_six_model_columns(html)
            html = ensure_toggle_pick_details_js(html)
        ...
        try:
            from live_proxy import strip_auth_chrome
    ...
    if sport == "ufc" and which == "picks":
        ...
        html = hide_unavailable_model_boxes(html)
    # Tennis/CFL/UFC — no reliable book ML; hide Books chrome (no dash columns).
    html = hide_empty_books_slots(html)
    if sport in ("tennis", "cfl", "ufc"):
    ...
    if which == "picks" and sport == "cfl":
        ...
        html = enrich_mlb_chart_data_attrs(html)
        html = inject_picks_chart_tabs(
            html, sport="cfl", markets=["moneyline", "spread", "totals"]
        )
    ...
    if which == "picks" and sport in ("ufc", "tennis"):
        ...
        # Kill inline MLB template chart Books columns (dashes when no book ML).
        html = re.sub(
            r"(?:const|var|let)\s+showBooksML\s*=\s*_hasRealBooksML\s*\(\s*\)\s*;",
            "const showBooksML = false;",
            html,
        )
...
def sport_picks_writeup_html(sport: str) -> str:
    ...
    heading_h2 = ""
    # Soccer/golf: AI Picks H1 + intro is the product title. A second
    # "⛳ Golf Predictions" / "⚽ Soccer Predictions" H2 duplicates it.
    if sport_l not in ("soccer", "golf"):
        heading_h2 = (
...
def apply_tennis_known_player_logos(html: str) -> str:
    ...
    if not html:
        return html
    try:
        from tennis_page import tennis_player_logo
        from tennis.espn_sync import load_headshot_cache
    except Exception:
        return html
    ...
    if names:
        try:
            ...
            with ThreadPoolExecutor(max_workers=6) as pool:
                futs = [pool.submit(tennis_player_logo, n) for n in names[:24]]
                for fut in as_completed(futs):
    ...
    # Keep scoreboard-cached ESPN ids (and any logo tennis_player_logo resolved).
    _known_ids = set(load_headshot_cache().values())
