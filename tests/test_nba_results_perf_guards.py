...
def test_nba_weekly_skips_v2_for_old_games(monkeypatch):
    import NHL77FINAL as N

    calls = []

    def _fake_v2(sport, home, away, game_date=None):
        calls.append((sport, home, away, game_date))
        return {
            'glicko2_prob': 0.55,
            'trueskill_prob': 0.54,
            'xgboost_prob': 0.56,
    ...
    monkeypatch.setattr(N, 'get_v2_prediction', _fake_v2)
    ...
    assert calls == [], 'v2 should not run for games older than 21 days when DB probs exist'
