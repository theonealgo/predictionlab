    ...
    def _audit_page(self, slug: str):
        ...
        if len(probs) >= 5:
            ...
            if pct_clustered >= CLUSTER_WARN_PCT:
                lvl = INFO if is_offseason(sport) else WARN
                self.r.add(CheckResult(
                    label=f"Model clustering: {sport}", status=lvl,
                    message=(f"{pct_clustered:.0f}% of probabilities are between "
                             f"{CLUSTER_RANGE_LOW}% and {CLUSTER_RANGE_HIGH}% — "
                             f"{'expected during offseason' if lvl == INFO else 'possible fallback model detected'}"),
                    url=f"/{slug}", auditor=self.NAME))
...
class ResultsAuditor:
    ...
    def run(self):
        ...
        # (n >= 20) is a red flag. Small samples are just variance and are
        # intentionally ignored (the results page itself marks n<20 records).
        ANOMALY_MIN_N = 20
        ...
        # Sub-50% model watch: flag models whose win-rate is STATISTICALLY
        # significantly below 50% (z-test, p < 0.05 → z < -1.96). Borderline
        # records like 49.4% over 700 games are normal variance, not a signal.
        import math
        SUB50_MIN_N = 20
        Z_THRESHOLD = -1.96
        under_50 = []
        for pct_str, w_str, l_str in pairs:
            ...
            pct = float(pct_str)
            if pct < 50.0:
                z = (pct / 100.0 - 0.5) / math.sqrt(0.25 / n)
                if z < Z_THRESHOLD:
        ...
        if under_50:
            self.r.add(CheckResult(
                ...
                message=f"{len(under_50)} model record(s) statistically below 50% "
                        f"(p<0.05) — losing money, consider flipping",
                detail=", ".join(under_50[:12]), auditor=self.NAME))
        else:
            self.r.add(CheckResult(
                label="Models under 50%", status=PASS,
                message=f"No model statistically below 50% over the {SUB50_MIN_N}+ game threshold",
                auditor=self.NAME))
        ...
        # ── "Not tracked" models in all-sports-results ──────────────────────
        not_tracked_count = html.lower().count('not tracked')
        if not_tracked_count:
            self.r.add(CheckResult(
                label="Models not tracked", status=WARN,
                message=f"{not_tracked_count} 'Not tracked' cell(s) on all-sports-results — "
                        f"model data missing for some sports",
                auditor=self.NAME))
        else:
            self.r.add(CheckResult(
                label="Models not tracked", status=PASS,
                message="All model cells populated on all-sports-results",
                auditor=self.NAME))

        # ── Season stats showing "—" ────────────────────────────────────────
        dash_cells = len(re.findall(r'>\s*—\s*<', html))
        if dash_cells > 6:
            self.r.add(CheckResult(
                label="Season stats blank", status=WARN,
                message=f"{dash_cells} blank '—' cells on all-sports-results — "
                        f"snapshot data may be missing (ROI, season record)",
                auditor=self.NAME))
        else:
            self.r.add(CheckResult(
                label="Season stats blank", status=PASS,
                message=f"Season stats populated ({dash_cells} dash cells within tolerance)",
                auditor=self.NAME))

        # ── Each sport's results page loads ─────────────────────────────────
        _fallback_phrases = ['no results data available', 'results will appear once',
                             'could not be loaded']
        empty_sports = []
        for slug in SPORT_RESULTS_SLUGS:
            sport_name = slug.replace('-results', '').upper()
            if is_offseason(sport_name):
                continue
            sport_html = self._fetch(f"/{slug}")
            if not sport_html or len(sport_html) < 3000:
                empty_sports.append(slug)
                continue
            for phrase in _fallback_phrases:
                if phrase in sport_html.lower():
                    empty_sports.append(slug)
                    break
        if empty_sports:
            self.r.add(CheckResult(
                label="Empty results pages", status=WARN,
                message=f"{len(empty_sports)} in-season sport results page(s) show "
                        f"no data or fallback message",
                detail=", ".join(empty_sports),
                auditor=self.NAME))
        else:
            self.r.add(CheckResult(
                label="Empty results pages", status=PASS,
                message="All in-season sport results pages have data",
                auditor=self.NAME))
        ...
        broken_imgs = []
        checked_imgs = 0
        seen_img_domains = set()
        for url in img_urls[:50]:
            if not url.startswith('http'):
                continue
            domain = urlparse(url).netloc
            if domain in seen_img_domains and checked_imgs > 10:
                continue
            seen_img_domains.add(domain)
            checked_imgs += 1
            try:
                img_resp = self.s.head(url, timeout=5, allow_redirects=True)
                if img_resp.status_code >= 400:
                    broken_imgs.append(f"{img_resp.status_code}: {url[:80]}")
            except Exception:
                broken_imgs.append(f"timeout: {url[:80]}")
        if broken_imgs:
            self.r.add(CheckResult(
                label="Broken team images", status=WARN,
                message=f"{len(broken_imgs)} broken image(s) on results page",
                detail="; ".join(broken_imgs[:5]),
                auditor=self.NAME))
        elif checked_imgs:
            self.r.add(CheckResult(
                label="Broken team images", status=PASS,
                message=f"Checked {checked_imgs} image URLs — all reachable",
                auditor=self.NAME))

        # ── Performance page load ───────────────────────────────────────────
        import time as _time_mod
        perf_start = _time_mod.time()
        perf_html = self._fetch("/performance")
        perf_elapsed = _time_mod.time() - perf_start
        if not perf_html or len(perf_html) < 2000:
            self.r.add(CheckResult(
                label="Performance page", status=WARN,
                message="Performance page did not load or returned minimal content",
                auditor=self.NAME))
        elif 'auto-refreshes' in perf_html.lower() or 'loading model performance' in perf_html.lower():
            self.r.add(CheckResult(
                label="Performance page", status=INFO,
            ...
            self.r.add(CheckResult(
                ...
                message=f"Performance page loaded in {perf_elapsed:.1f}s",
                auditor=self.NAME))
...
class PropsAuditor:
    ...
    def _audit_diagnostics(self, sport: str, diag: dict):
        ...
        if fi and len(nonzero) <= 1:
            lvl = INFO if offseason else WARN
            self.r.add(CheckResult(
                label=f"{sport} diagnostics: feature collapse", status=lvl,
                message=f"Only {len(nonzero)} feature(s) carry signal "
                        f"({', '.join(nonzero) or 'none'}) — "
                        f"{'expected during offseason' if offseason else 'feature importance is degenerate'}",
                auditor=self.NAME))
        ...
        if stab == "degenerate":
            lvl = INFO if offseason else WARN
            self.r.add(CheckResult(
                label=f"{sport} diagnostics: degenerate distribution", status=lvl,
                message=f"All pick probabilities clustered near 50% — "
                        f"{'expected during offseason' if offseason else 'possible fallback model'}",
                auditor=self.NAME))
