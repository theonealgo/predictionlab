...
def _six_model_chart_broken(html: str) -> bool:
    ...
    html = html or ""
    if re.search(r"among the [45] live models|[45] live models", html, flags=re.I):
        return True
...
def render_nfl_results_chart_page(payload=None, *, market=None):
    """NFL /nfl-results?view=chart using the shared team_results template."""
    ...
    html = env.get_template("team_results.html").render(
        ...
        api_base="/nfl/api",
        show_league=False,
        picks_href="/nfl-picks",
        results_href="/nfl-results",
    )
    bar = (
        '<div class="pl-view-toggle" role="navigation" aria-label="Results view">'
        '<a class="pl-view-btn" href="/nfl-results">Cards</a>'
        '<a class="pl-view-btn active" href="/nfl-results?view=chart">Chart</a>'
        "</div>"
        "<style>.pl-view-toggle{display:flex;gap:8px;margin:12px 16px 18px;flex-wrap:wrap}"
        ".pl-view-btn{display:inline-flex;align-items:center;padding:8px 14px;border-radius:999px;"
        "border:1px solid #dbe3ee;background:#fff;color:#0c1e3a;font-weight:700;font-size:.85rem;"
    ...
    if 'class="pl-view-toggle"' not in html and "class='pl-view-toggle'" not in html:
        import re as _re

        if _re.search(r"<main\b", html, _re.I):
            html = _re.sub(r"(<main\b[^>]*>)", r"\1" + bar, html, count=1, flags=_re.I)
        else:
            html = bar + html
    ...
    if payload:
        try:
            ...
            try:
                html = inject_ssr_chart_bootstrap(html, payload, "nfl", market=market)
            except TypeError:
                html = inject_ssr_chart_bootstrap(html, payload, "nfl")
        except Exception as e:
            print(f"[team_results_charts] NFL chart SSR: {e}", flush=True)
    return html
